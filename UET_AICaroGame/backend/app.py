...
host = 'http://127.0.0.1:8080'  # Địa chỉ server trọng tài mặc định
...
class GameClient:
    def __init__(self, server_url, room_id, your_team_id, opponent_team_id,  your_team_role):
        self.server_url = server_url
        self.team_id = f'{your_team_id}+{your_team_role}'
        self.your_team_id = your_team_id
        self.opponent_team_id = opponent_team_id
        self.team_role = your_team_role
        self.match_id = None
        self.board = None
        self.init = None
        self.size = None
        ...
        while not stop_thread:
            ...
            # Nhận thông tin trò chơi
                ...
                if data.get("turn") in self.team_id:
                    ...
                    move = get_move(self.board, self.size, self.team_role)
                    print("Move: ", move)
                    # Kiểm tra nước đi hợp lệ
                    valid_move = self.check_valid_move(move)
                    # Nếu hợp lệ thì gửi nước đi
                    if valid_move:
                        self.board[int(move[0])][int(move[1])] = self.team_role
...
if __name__ == "__main__":
    ...
    team_role = input("Enter your team role (x/o): ").lower()
    # Khởi tạo game client
    gameClient = GameClient(host, room_id, your_team_id, opponent_team_id, team_role)
    gameClient.listen()
