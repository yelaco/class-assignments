...
def f(x: float) -> float:
    return x * np.sin(x) + x**2 * np.cos(x)


def grad_f(x: float) -> float:
    return np.sin(x) + x * np.cos(x) + 2 * x * np.cos(x) - x**2 * np.sin(x)


if __name__ == "__main__":
    x_ = np.array([1.1, 1.2, 1.3, 1.4], np.float64)
    f_ = np.array([1.52918, 1.64024, 1.70470, 1.71277], np.float64)
    h = x_[1] - x_[0]
